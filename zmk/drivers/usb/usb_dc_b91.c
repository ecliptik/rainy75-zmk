...
enum b91_usb_diag_code {
	...
	B91_DIAG_WSTALL   = 15, /* a = slot idx, b = consecutive ticks its k_work
				 * sat QUEUED without running (any endpoint, so
				 * this catches TX-side wedges too) */
	B91_DIAG_WDETAIL  = 16, /* a = slot idx, b = raw k_work flags low 16 */
	B91_DIAG_WQPTR    = 17, /* a = slot idx, b bit0: work->queue == usb wq,
...
/* ---------------------------------------------------------------------- *
 *  Transfer-layer stall detector                                          *
 *                                                                         *
 *  Every wedge captured so far ends with a transfer slot whose k_work is   *
 *  flagged QUEUED while the workqueue is provably running (the marker      *
 *  above keeps ticking).  A queued completion that does not run within     *
 *  seconds is always pathological, on any endpoint — which is what makes   *
 *  this a TX-side detector too: the 2026-07-24 wedge stranded the CDC      *
 *  bulk IN slot, invisible to the bulk-OUT arm-state watchdog, so it       *
 *  needed a physical replug.                                              *
 * ---------------------------------------------------------------------- */

#define B91_WSTALL_SLOTS 4
#define B91_WSTALL_TICKS 20     /* x 500 ms = 10 s of continuously QUEUED */
/* Re-notify while a stall persists (the recovery module applies its own
 * holdoff and escalates from re-attach to reboot). */
#define B91_WSTALL_RETRY_TICKS 240      /* x 500 ms = 2 min */

static uint16_t wstall_ticks[B91_WSTALL_SLOTS];
static bool wstall_fired;
static uint16_t wstall_retry_ticks;

/* Dump raw k_work state for every slot: flags, owning queue, and whether the
 * node is really on that queue's pending list.  QUEUED set with the node
 * absent from the list is flag/list divergence — the item can never run. */
static void wstall_capture_details(void)
{
	for (size_t i = 0; i < B91_WSTALL_SLOTS; i++) {
		struct k_work *w = usb_transfer_slot_work(i);

		if (w == NULL) {
			break;
		}

		diag_ev(B91_DIAG_WDETAIL, (uint8_t)i, (uint16_t)w->flags);
...
static void wstall_tick(void)
{
	if (state.suspended ||
	    !(usb_read8(B91_USB_HOST_CONN) & B91_USB_HOST_CONN_CONFIGURED)) {
		memset(wstall_ticks, 0, sizeof(wstall_ticks));
		wstall_fired = false;
		return;
	}
	...
	for (size_t i = 0; i < B91_WSTALL_SLOTS; i++) {
		...
		if (k_work_busy_get(w) & K_WORK_QUEUED) {
			if (wstall_ticks[i] < UINT16_MAX) {
				wstall_ticks[i]++;
			}
			if (wstall_ticks[i] >= B91_WSTALL_TICKS) {
				stalled = true;
				if (!wstall_fired) {
					diag_ev(B91_DIAG_WSTALL, (uint8_t)i,
						wstall_ticks[i]);
					LOG_ERR("transfer slot %u queued %u "
						"ticks without running",
						(unsigned)i, wstall_ticks[i]);
				}
			}
		} else {
			wstall_ticks[i] = 0;
	...
	if (stalled) {
		if (!wstall_fired) {
			wstall_fired = true;
			wstall_capture_details();
			b91_usb_cdc_starved();
		} else if (++wstall_retry_ticks >= B91_WSTALL_RETRY_TICKS) {
			/* Re-arm rather than latch: if the first recovery does
			 * not clear the stall, the hook must be called again so
			 * the ladder can escalate to a reboot.  Latching here
			 * left a wedged board waiting for a physical replug
			 * (observed 2026-07-24). */
			wstall_retry_ticks = 0;
			b91_usb_cdc_starved();
		}
	} else {
		wstall_fired = false;
